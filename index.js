...
const lookup = async (word) => {
  ...
  const result = await (await fetch(`https://dictionaryapi.com/api/v3/references/collegiate/json/${word}?key=${mwKey}`)).json();
  cache[word] = result;
  return result;
};
...
(async () => {
  try {
    ...
    console.log(response.ok ? 'Command set!' : `Command error: ${text}`);
  } catch (err) {
